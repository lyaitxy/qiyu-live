...
public class PayProductServiceImpl implements IPayProductService {
    ...
    public List<PayProductDTO> products(Integer type) {
        ...
        List<PayProductDTO> payProductDTOList = redisTemplate.opsForList().range(cacheKey, 0, 30).stream().map(x -> (PayProductDTO) x).collect(Collectors.toList());
        if (!CollectionUtils.isEmpty(payProductDTOList)) {
            if (payProductDTOList.get(0) == null) {
                return Collections.emptyList();
            }
            return payProductDTOList;
        }
